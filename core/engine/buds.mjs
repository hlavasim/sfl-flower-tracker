...
import {
  ...
  getCycleSec, getCapacityCount, getDefaultProduct, getBaseYield,
    ...
    /*
     * The game's own boost taxonomy: Marketplace → Bud NFTs → Boost, verbatim and in its
     * order (captured 2026-07-29). Each entry lists the traits that grant that boost.
     *
     * This is the list players actually shop by, and it is NOT the same as flattening
     * `cats` out of the tables above. "Minerals" and "Stone" are separate filters — Cave and
     * Diamond Gem boost all three minerals together, Ruby Gem boosts only stone — while cows
     * and sheep do not exist as filters at all, because no trait targets them on their own
     * (Retreat is "Animal Produce"). Crimstone is absent for the same reason: no bud boosts
     * it, `isMineral` in the game is Stone | Iron | Gold.
     *
     * Pinned by a test against BUD_TYPE_BOOSTS/BUD_STEM_BOOSTS: every trait that carries a
     * monetary boost appears here exactly once, and every trait named here exists.
     */
    const BUD_BOOST_FILTERS = {
      "Basic Crops":          { types: ["Plaza"],     stems: ["Basic Leaf"] },
      "Medium Crops":         { types: ["Castle"],    stems: [] },
      "Advanced Crops":       { types: ["Snow"],      stems: [] },
      "All Crops":            { types: [],            stems: ["3 Leaf Clover"] },
      "Crop Growth Time":     { types: ["Saphiro"],   stems: [] },
      "Carrot":               { types: [],            stems: ["Carrot Head"] },
      "Sunflower":            { types: [],            stems: ["Sunflower Hat"] },
      "Fruit":                { types: ["Beach"],     stems: ["Apple Head", "Banana"] },
      "Mushroom":             { types: [],            stems: ["Mushroom"] },
      "Magic Mushroom":       { types: [],            stems: ["Magic Mushroom"] },
      "Wood":                 { types: ["Woodlands"], stems: ["Acorn Hat", "Tree Hat"] },
      "Stone":                { types: [],            stems: ["Ruby Gem"] },
      "Iron":                 { types: [],            stems: ["Miner Hat"] },
      "Gold":                 { types: [],            stems: ["Gold Gem"] },
      "Minerals":             { types: ["Cave"],      stems: ["Diamond Gem"] },
      "Egg":                  { types: [],            stems: ["Egg Head"] },
      "Animal Produce":       { types: ["Retreat"],   stems: [] },
      "Fish":                 { types: ["Sea"],       stems: ["Fish Hat"] },
      // Port's only boost is +10% Fish XP — real in game, worth 0 FLOWER, so a wishlist row
      // for it reports "—" rather than being hidden.
      "Bumpkin XP from Fish": { types: ["Port"],      stems: [] },
    };

    /** Does this decoded bud carry the named game boost filter? */
    function budHasBoostFilter(bud, filterName) {
      const g = BUD_BOOST_FILTERS[filterName];
      if (!g || !bud) return false;
      return g.types.includes(bud.type) || g.stems.includes(bud.stem);
    }
