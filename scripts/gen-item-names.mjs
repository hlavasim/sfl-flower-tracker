// Generates api/_item-names.js — the marketplace id -> name mapping.
//
...
// mislabel items.
//
// Lives OUTSIDE api/ on purpose: Vercel counts every .js/.mjs under api/ (including
// subdirectories) as a serverless function, and the Hobby plan caps the project at 12 —
// which it was already at, so a generator sitting in api/scripts/ failed the deployment.
//
// Emitted as a .js module rather than .json: Vercel compiles these functions from ESM to
// CommonJS, and an `import ... with { type: "json" }` does not reliably survive that. A
// plain module is a normal import, so it transpiles and gets bundled without special cases.
//
// Regenerate when the game adds items:
//   node scripts/gen-item-names.mjs
